...
class TestUiPayrollAgreementInputType(HttpSavepointCase):
    """Tour tests for the ``payroll_agreement_input_type`` work
    instructions.
    """
    ...
    def setUpClass(cls):
        ...
        active ``sequence.template`` for ``payroll_agreement_input_type``,
        with ``initial_string`` ``/`` so the tour's own ``/`` entry in
        **Code** is eligible for generation. No ``sequence.template``
        data record is added to the module itself -- this fixture is
        test-only.
        """
        super().setUpClass()
        ...
        sequence = (
            ...
            .create(
                {
                    "name": "Tour Payroll Agreement Input Type Sequence",
                    "prefix": "TOURPAIT-",
                    "padding": 4,
                }
            )
        ...
        model = cls.env["ir.model"].search(
            [("model", "=", "payroll_agreement_input_type")], limit=1
        )
    ...
    def test_create(self):
        ...
        setup is needed. Pre-Condition IK (Config) is set up in
        :meth:`setUpClass`.
