...
>
> **Extends:** ssi_payroll_agreement — model `payroll_agreement`, aksi `05-approve`
...
- The behavior below only applies **when the record's active Approval Template has a
  ...
  `ssi_payroll_agreement/docs/payroll_agreement/05-approve.md`.
...
- Instead of clicking Approve, the user opens the linked request — shown under the
  ...
  **Signature Requests** tab (or via the **Open Signature Requests** button in the same
  ...
  Odoo, in Documenso.
- Approval completes automatically — without any button click on this Payroll Agreement
  ...
  automatically changes to **Ready to Process** (or remains **Waiting for Approval** if
  other approval levels are still pending).
